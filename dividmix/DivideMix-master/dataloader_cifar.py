        ...
        if self.mode=='test':
            ...
            if os.path.exists(noise_file):
                ...
                for i in range(50000):
                    if i in noise_idx:
                        if noise_mode=='sym':     #产生symmetric噪声，等可能地随机转为任一label
                            if dataset=='cifar10': 
                                noiselabel = random.randint(0,9)
                            elif dataset=='cifar100':    
                                noiselabel = random.randint(0,99)
                            noise_label.append(noiselabel)
                        elif noise_mode=='asym':    #产生asymmetric噪声
            ...
            if self.mode == 'all':
                ...
                if self.mode == "labeled":
                    pred_idx = pred.nonzero()[0]            #得到labeled data对应下标
                    self.probability = [probability[i] for i in pred_idx]   #得到被认为是labeled data对应的概率
                    
                    clean = (np.array(noise_label)==np.array(train_label))        #认为是labeled实际不一定是labeled的，可以通过跟实际noise_label对照绘制出AUC曲线查看划分性能                                             
                    auc_meter = AUCMeter()
                    auc_meter.reset()
